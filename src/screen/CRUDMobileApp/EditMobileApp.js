...
const EditMobileApp = () => {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    await axios.put(`http://localhost:3001/mobileApps/${id}`,  {appName, appImage, appDetail}); // Data update
  ...
  const loadUser = async () => {
    const result = await axios.get(`http://localhost:3001/mobileApps/${id}`); // Selected id data show
  ...
  return (
    ...
    <>
      ...
      <Banner
        title="Edit Mobile Application"
        image={bg}
      />

      <div className="container pb-50">

        <Heading titleGray="Edit App" titleBlack="Edit" titleBlue="Mobile Application" />
        ...
        <div className="w-75 mx-auto shadow p-5 pt-0">
          <form onSubmit={(e) => onSubmit(e)}>
            ...
            <button className="btn btn-warning btn-block">Update Mobile App</button>
          </form>
        </div>
      </div>
